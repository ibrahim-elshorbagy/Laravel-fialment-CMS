...
<div x-data="{ preview: false, content: @entangle($attributes->wire('model')) }"
     class="p-4 bg-white border rounded-lg shadow-sm wysiwyg-editor dark:border-0 dark:bg-gray-900"
     id="editor-container-{{ $editorId }}">
    <div class="toolbar flex space-x-2 mb-4 {{ $options ? 'dark:border-0 border-b' : '' }} pb-2">
        @foreach ($options as $option)
            <button type="button"
                    class="px-2 py-1 text-gray-700 rounded hover:bg-gray-200 dark:border-0 dark:hover:bg-gray-800 dark:text-gray-200 focus:outline-none"
